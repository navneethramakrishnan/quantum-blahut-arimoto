function plot_CI_amplitude_damping_one_curve()

clear; 
load('data_CI_amplitude_damping.mat');
savefile = 1;
...
box on;
hold on;
plot(iter, capacity/log(2), 'r.-', 'LineWidth', 3, 'MarkerSize', 40);

%Axis label
xlabel('Number of iterations, $n$', 'FontSize', 40, 'Interpreter', 'latex');
ylabel('Coherent information estimate (bits), $I_{\rm{coh}}^{(n)}$', 'FontSize', 40, 'Interpreter', 'latex');
...
set(gca,'YTick',[0.3265 0.3270 0.3275 0.3280]);
...
if savefile
    ...
    % Save the pdf
    print -dpdf CI_amplitude_damping_one_curve;
