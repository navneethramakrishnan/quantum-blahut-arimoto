function[c,iter,state,F] = CI_amplitude_damping(adaptive)
%Input dimensions
dim = 2;
...
capacity_prev = findCapacity_CI(Nrho_prev, complementNrho_prev);
convergence = 0;
counter = 1;
...
while ~convergence 
    ...
    capacity_curr = findCapacity_CI(Nrho_curr, complementNrho_curr);
    convergence = CI_convergence_test(capacity_curr, C-B, tol);
    if adaptive 
        gamma = (quantum_rel_entr(Nrho_curr, Nrho_prev) - quantum_rel_entr(complementNrho_curr, complementNrho_prev))/quantum_rel_entr(rho_curr, rho_prev); %Adaptive step size
        gamma(isnan(gamma)) = 1;
        gamma(gamma==0) = 1;
    end
    ...
    F{counter} = adjlogcomplementNrho_curr - adjlogNrho_curr;
